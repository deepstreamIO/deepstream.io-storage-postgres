...
describe('connector', () => {
  ...
  describe.skip('creates multiple connectors in parallel', () => {
  ...
  describe( 'creates databases', () => {
    ...
    it( 'creates a schema', ( done ) => {
      dbConnector.createSchema('some-schema', (err) => {
    ...
    it( 'deletes a schema', ( done ) => {
      dbConnector.destroySchema( 'some-schema', (err) => {
        expect( err ).to.equal( null )
        done()
      })
    })
  ...
  describe( 'sets and gets values', () => {
    ...
    const messages: any[] = []
    const ITEM_NAME = 'some-boring-table/some-key'
    ...
    it('deletes the possible schema', (done) => {
      setTimeout(() => {
        dbConnector.destroySchema('test', () => {})
        done()
      }, 1000)
    })

    it('creates the possible schema', (done) => {
      setTimeout(() => {
        dbConnector.createSchema('test', () => {})
        done()
      }, 1000)
    })
    ...
    it('received a notification', () => {
      ...
      expect( messages[ 0 ] ).to.deep.equal({ event: 'CREATE_TABLE', table: 'some-boring-table' })
      expect( messages[ 1 ] ).to.deep.equal({ event: 'INSERT', table: 'some-boring-table', key: 'some-key' })
    })
    ...
    it( 'sets a value for an existing table', ( done ) => {
      expect( messages.length ).to.equal( 2 )
      dbConnector.set( 'some-boring-table/another-key', 10 , { firstname: 'Egon' }, ( error ) => {
        expect( error ).to.equal( null )
        // give it some time to receive the notifications
        setTimeout( done, 300 )
      })
    ...
    it( 'received a notification', () => {
      ...
      expect( messages[ 2 ] ).to.deep.equal({ event: 'INSERT', table: 'some-boring-table', key: 'another-key' })
    })
    ...
    it( 'received a notification', () => {
      ...
      expect( messages[ 3 ] ).to.deep.equal({ event: 'DELETE', table: 'some-boring-table', key: 'some-key' })
    })
  ...
  describe.only( 'advanced sets', () => {
    ...
    it( 'writes multiple values in quick succession to an existing table', ( done ) => {
      dbConnector.set( 'some-boring-table/itemA', 1, { val: 1 }, () => {})
      dbConnector.set( 'some-boring-table/itemA', 1, { val: 2 }, () => {})
      dbConnector.set( 'some-boring-table/itemA', 1, { val: 3 }, () => {})
      dbConnector.set( 'some-boring-table/itemA', 1, { val: 4 }, () => {})
      dbConnector.set( 'some-boring-table/itemA', 1, { val: 5 }, ( error ) => {
        expect( error ).to.equal(null)
        done()
      })
    })
    ...
    it( 'retrieves the latest item from the last operation', ( done ) => {
      dbConnector.get( 'some-boring-table/itemA', ( error, version, item ) => {
        expect( error ).to.equal(null)
    ...
    it( 'received the right notifications', () => {
      ...
      expect( messages[ 0 ].table ).to.equal( 'some-boring-table' )
    ...
    //   messages = []
    //   dbConnector.set( 'some-boring-table/x1', 1, { val: 42 }, () => {})
    ...
    //   messages = []
    //   dbConnector.set( 'some-boring-table/x2', 1, { val: 43 }, ( err, result ) => {
    //     expect( err ).to.be.null
    //   })
    //   setTimeout(() => {
